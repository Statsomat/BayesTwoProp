...
df_table <- as.data.frame.matrix(matrix(c(
  ...
  n_exposure0 - n_exposure0_outcome1+n_exposure1 - n_exposure1_outcome1, # sum no outcome
  n_exposure0_outcome1,               # Not exposed with outcome
  n_exposure1_outcome1,                # Exposed with outcome
  n_exposure0_outcome1+n_exposure1_outcome1,
  n_exposure0,
  n_exposure1,
  n_exposure0+n_exposure1
), nrow = 3, byrow = TRUE))
colnames(df_table) <- c("Placebo", "Vaccine","sum")
rownames(df_table) <- c("non-outcome", "outcome","sum")

total <- n_exposure0+n_exposure1
...
df_table_percent<- as.data.frame.matrix(matrix(c(
  paste0((n_exposure0 - n_exposure0_outcome1)/total*100,"%"), 
  paste0((n_exposure1 - n_exposure1_outcome1)/total*100,"%"),
  paste0((n_exposure0 - n_exposure0_outcome1+n_exposure1 - n_exposure1_outcome1)/total*100,"%"), 
  paste0((n_exposure0_outcome1)/total*100,"%"), 
  paste0((n_exposure1_outcome1)/total*100,"%"),                
  paste0((n_exposure0_outcome1+n_exposure1_outcome1)/total*100,"%"),
  paste0((n_exposure0)/total*100,"%"),
  paste0((n_exposure1)/total*100,"%"),
  paste0(100,"%")
), nrow = 3, byrow = TRUE))
colnames(df_table_percent) <- c("Placebo", "Vaccine","sum")
rownames(df_table_percent) <- c("non-outcome", "outcome","sum")

df_table_percent %>%
  kbl(caption = "Contingency Table: percents") %>%
  kable_styling(full_width = FALSE, position = "center") %>%
  row_spec(0, extra_css = "font-weight: normal; border-bottom: 2px solid;") %>%
  row_spec(nrow(df_table) - 1, extra_css = "border-bottom: 2px solid;") %>%   
  column_spec(1, extra_css = "border-right: 2px solid black;") %>%         
