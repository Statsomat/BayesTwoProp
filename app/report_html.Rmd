...
author: 
  - Denise Welsch
  - Contributors^[Viktoria Daum, Chiara Freitag, Konrad Junkes, Anna-Lena Künster, Jomana Reusch, Simone Schüttler]
...
thetaoptions <- c("$\\theta_1 - \\theta_2$","$\\frac{\\theta_1}{\\theta_2}$","$1-\\frac{\\theta_1}{\\theta_2}$")
hdi_result <- hdi(estimate, ci = c(.89, .95, .99))

# Display the HDI plot
hdi_plot <- plot(hdi_result) +
  labs(
    x = cat(thetaoptions[params$user_selection_function_param], fill=TRUE ),                 # Label for the x-axis
...
cat("\n\n**Interpretation:**\n\n")
cat("The Highest Density Interval (HDI) plot visualizes the parameter's posterior distribution. The shaded regions indicate the intervals with the highest density, representing the most credible parameter values under Bayesian analysis.")
