...
final_results <-  data.frame(Parameter = final_results[,1], Posterior=final_results[,2], Interpretation = interpretation_func(final_results, parameter_description))
...
```{r}
# erstelle temporären Dataframe, um Probleme zu vermeiden, falls im weiteren Verlauf final_results referenziert wird
final_results_temp <- final_results

final_results_temp[,1] <- c(
  "Credibility Interval (HDI)",
  "Lower Bound of HDI",        
  "Upper Bound of HDI"         
)

colnames(final_results_temp) <- c("Parameter", "Posterior")

kable(final_results_temp,
      align = "l",
      caption = "HDI Table",
      row.names = FALSE) %>%
  kable_styling(full_width = TRUE,
...
# entfernnen des temporären Dataframes
rm(final_results_temp)
```
