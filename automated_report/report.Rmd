...
author: 
  - Denise Welsch
  - Contributors^[Viktoria Daum, Konrad Junkes, Simone Schüttler]
...
```{r desctable, eval=eval_final, dev="cairo_pdf", results='asis'}
cat("## Frequency Table", fill=TRUE)
# Insert a nice frequency and percentage table for the data
```

```{r descplots, eval=eval_final, dev="cairo_pdf", results='asis'}
...
cat("$$X_{11},...,X_{1n_1} \\hspace{1cm} iid \\text{ \\~{ } } Be(\\theta_1)$$", fill=TRUE)
cat("$$X_{21},...,X_{2n_2} \\hspace{1cm} iid \\text{ \\~{ } } Be(\\theta_2)$$", fill=TRUE)

cat(paste0("with sample sizes $n_1$ and $n_2$, and Bernoulli distributed random variables in the two groups with distribution parameter $\\theta_i \\, , i\\in\\{1,2\\}$.\\
The exposure variable indicates in which of the two groups (",colnames(table(params$data))[1]," and ",colnames(table(params$data))[2], ") an individual is. Whether the event (",colnames(params$data)[1],") applies to an individual is indicated by the outcome variable, which is either 0 or 1.\\"), fill=TRUE)


#cat(paste0("with sample sizes $n_1$ and $n_2$, and Bernoulli distributed random variables in the two groups with distribution parameter $\\theta_i \\, , i\\in\\{1,2\\}$.\\
#The data consist of the two groups  (",colnames(table(params$data))[1]," and ",colnames(table(params$data))[2], "), where the membership of an individual in one of the groups corresponds to the exposure variable. Each individual has the value 1 or 0, this is the outcome variable and indicates whether the event applies to the individual or not."), fill=TRUE)

#cat("This data consists of two groups (e.g. vaccinated and not vaccinated) and contains for the respective individual a 1 or a 0, depending on whether the event (e.g. ill) applies to the respective individual or not. The two groups $X_{1i}$ and $X_{2i}$ are thus Bernoulli distributed with the respective distribution parameter $\\theta_i \\, , i\\in\\{1,2\\}$. ", fill=TRUE)
#cat("which are a sample of Bernoulli distributed values with distribution parameter $\\theta_i$.", fill=TRUE)
#cat("The statistical model of this analysis derives from the data: ", fill=TRUE)
cat("The model then results from the respective summation of the values 0 and 1 in the two groups.", fill=TRUE)
cat("$$Y_1= \\sum_{i=1}^{n_1} X_{1i} \\hspace{1cm} iid \\text{ \\~{ } } Bi(n_1,\\theta_1)$$", fill=TRUE)
cat("$$Y_2= \\sum_{i=1}^{n_2} X_{2i} \\hspace{1cm} iid \\text{ \\~{ } } Bi(n_2,\\theta_2)$$", fill=TRUE)
cat("This models are binomially distributed with the distribution parameters $n_i$ and $\\theta_i \\, , i\\in\\{1,2\\}$. ", fill=TRUE)

if(user_selection_function_param==1){
  cat("Theta1-Theta2", fill=TRUE)
}else{
...
### work over here 
#cat("Theta1-Theta2 Theta1/Theta2 1-Theta1/Theta2 with Theta1-Theta2 as default", fill=TRUE)
...
# Display and explain the user selected function of parameters 
```
...
```{r posterior, eval=eval_final, dev="cairo_pdf", results='asis'}
...
```{r posterior_check, eval=eval_final, dev="cairo_pdf", results='asis'}
...
```{r plot_hdi, eval=eval_final, dev="cairo_pdf", results="asis"}
...
```{r plot_eti, eval=eval_final, dev="cairo_pdf", results="asis"}
