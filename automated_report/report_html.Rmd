...
cat("The likelihood function describes the plausibility of the observed data $(Y_1,Y_2)$ given the parameters $\\theta_1$, $\\theta_2$. The likelihood function should not be confused as a probability function.", fill = TRUE)
cat("<br>", fill=TRUE)
cat("$$(Y_1,Y_2) \\sim P(y_1,y_2|\\theta_1,\\theta_2)$$")
cat("<br>", fill=TRUE)
#Likelihood formula of D given theta_1 and theta_2
cat("$$P(Y_1=y_1,Y_2=y_2 | \\theta_1, \\theta_2) = \\begin{pmatrix} n_1 \\\\ y_1 \\end{pmatrix} \\theta_1^{y_1}(1-\\theta_1)^{n_1 - y_1} \\cdot  \\begin{pmatrix} n_2 \\\\ y_2 \\end{pmatrix} \\theta_2^{y_2}(1-\\theta_2)^{n_2 - y_2}$$")
cat("<br>", fill=TRUE)
cat("$$=L(\\theta_1, \\theta_2|y_1,y_2)$$", fill=TRUE)
cat("<br>", fill=TRUE)
#Likelihood formular of D given theta_1 and theta_2 with user entered data
cat("Here is an example with the following values:",fill=TRUE)
