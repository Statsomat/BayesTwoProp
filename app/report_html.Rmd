...
author: 
  - Denise Welsch
  - Contributors^[Theresa Binot, Viktoria Daum, Renée Defren, Chiara Freitag, Konrad Junkes, Anna-Lena Künster, Jomana Reusch, Simone Schüttler]
...
df_table_percent<- as.data.frame.matrix(matrix(c(
  paste0((n_exposure0 - n_exposure0_outcome1)/totalsum*100,"%"), 
  paste0((n_exposure1 - n_exposure1_outcome1)/totalsum*100,"%"),
  paste0((n_exposure0 - n_exposure0_outcome1+n_exposure1 - n_exposure1_outcome1)/totalsum*100,"%"), 
  paste0((n_exposure0_outcome1)/totalsum*100,"%"), 
  paste0((n_exposure1_outcome1)/totalsum*100,"%"),                
  paste0((n_exposure0_outcome1+n_exposure1_outcome1)/totalsum*100,"%"),
  paste0((n_exposure0)/totalsum*100,"%"),
  paste0((n_exposure1)/totalsum*100,"%"),
  paste0(100,"%")
), nrow = 3, byrow = TRUE))
...
cat("This two tables depict the frequency and the frequency in percentage of the whole data. One can also calculate the row percent and the column percent per table.")
cat("<br>", fill=TRUE)
##row percent
cat(paste0("Estimator for row percent: \\begin{align} P(\\text{",name_nonexposure,"} \\ | \\ \\text{ ",name_nonoutcome,"} ) &= ",round((n_exposure0 - n_exposure0_outcome1)/rowsum_1 * 100,digits = 1),"\\% \\\\
P(\\text{",name_exposure,"} \\ | \\ \\text{ ",name_nonoutcome,"} ) &= ",round((n_exposure1 - n_exposure1_outcome1)/rowsum_1 * 100,digits = 1),"\\% \\\\ 
\\P(\\text{",name_nonexposure,"} \\ | \\ \\text{ ",name_outcome,"} ) &= ",round((n_exposure0_outcome1)/rowsum_2 * 100,digits = 1),"\\% \\\\
\\P(\\text{",name_exposure,"} \\ | \\ \\text{ ",name_outcome,"} ) &= ",round((n_exposure1_outcome1)/rowsum_2 * 100,digits = 1),"\\% \\end{align}"))


##column percent
cat(paste0("Estimator for column percent: \\begin{align} P(\\text{",name_nonoutcome,"} \\ | \\ \\text{ ",name_nonexposure,"} ) &= ",round((n_exposure0 - n_exposure0_outcome1)/n_exposure0 * 100,digits = 1),"\\% \\\\
P(\\text{",name_outcome,"} \\ | \\ \\text{ ",name_nonexposure,"} ) &= ",round((n_exposure1 - n_exposure1_outcome1)/n_exposure0 * 100,digits = 1),"\\% \\\\ 
P(\\text{",name_nonoutcome,"} \\ | \\ \\text{ ",name_exposure,"} ) &= ",round((n_exposure0_outcome1)/n_exposure1 * 100,digits = 1),"\\% \\\\
P(\\text{",name_outcome,"} \\ | \\ \\text{ ",name_exposure,"} ) &= ",round((n_exposure1_outcome1)/n_exposure * 100,digits = 1),"\\% \\end{align}"))
...
eti_result = eti(estimate)


# Display the ETI plot 
eti_plot <- plot(eti_result) +
  labs(
    x = "Parameter",                 # Label for the x-axis
...
cat("\n\n**Interpretation:**\n\n")
cat("The Expected Tail Interval (ETI) plot visualizes the parameter's posterior distribution. The density curve highlights where the posterior probabilities are concentrated, and the shaded area (if present) illustrates the credible interval. This interval provides insights into the parameter's uncertainty in Bayesian analysis.\n")
```
