...
if(a1!=0.5 | b1!=0.5 | a2!=0.5 | b2!=0.5){
  ...
  if(a1 == a2 & a1 == b1 & a1 == b2){
    ...
    max_value <- max(pTheta1Theta2)
    max_position <- which(pTheta1Theta2 == max_value, arr.ind = TRUE)
    t1_m = round(Theta1[max_position[1]],digits = 3)
    t2_m = round(Theta2[max_position[2]],digits = 3)
  ...
  cat("These default parameters have their origin in the so called Jeffrey's prior which is a non-informative prior distribution. Non-informative prior distributions are used when little or no information is available about the parameters to be determined (here: $\\theta_1,\\theta_2$, in general: $\\theta$) and therefore no statement can be made about the distribution of the parameters. The priors $p(\\theta)=beta(0.5,0.5)$ result from Jeffrey's rule: $$p(\\theta) \\propto \\sqrt{I(\\theta)}$$ where $I(\\theta)$ is the Fisher information function $$I(\\theta)=-E\\left[ \\frac{\\partial^2}{\\partial \\theta^2} f(\\bullet \\ | \\ \\theta) \\right]$$ and where $f(\\bullet \\ | \\ \\theta)$ is the likelihood function of the data $\\bullet$.",fill=TRUE)
  cat("<br>", fill=TRUE)
  cat("In our case, the likelihood is a binomial distribution. Hence, the Fisher information function is $$I(\\theta)=\\frac{n}{\\theta (1-\\theta)} \\, .$$ This results in $$p(\\theta) \\propto \\theta^{-0.5}(1-\\theta)^{-0.5}$$ because of Jeffrey's rule. The resulting prior distribution is now proportional to $beta(0.5,0.5)$.",fill=TRUE)
...
cat("<br>", fill=TRUE)
cat("<br>", fill=TRUE)
cat("Notice that for the parameters $a$ and $b$ of an arbitrary $beta(a,b)$ distrbution we have: If $a>1$ and $b>1$, mean equals mode, and if $a=b$, mean and mode equal 0.5.",fill=TRUE)
