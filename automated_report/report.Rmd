...
author: 
  - Denise Welsch
  - Contributors^[Viktoria Daum, Konrad Junkes, Felix Nisius, Simone Schüttler]
...
# for example n_no_exposed_outcome is the frequency of datapoints without exposure but with present outcome

n_exposed_outcome = sum(df[outcome]==presence_outcome & df[exposure]==presence_exposure)
n_exposed = sum(df[exposure]==presence_exposure)
n_no_exposed_outcome = sum(df[outcome]== presence & df[exposure]!=presence_exposure)
n_no_exposed = sum(df[exposure]!=presence_exposure)
