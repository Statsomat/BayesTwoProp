...
author: 
  - Denise Welsch
  - Contributors^[Viktoria Daum, Konrad Junkes, Jomana Reusch, Simone Schüttler]
...
cat(paste0("Consider our data $n_1=", n1, ", n_2=", n2, ", y_1=", s1, ", y_2=", s2,"$:"),fill=TRUE)
cat("<br>", fill=TRUE)
cat(paste0("$$P(Y_1=", s1, ",Y_2=", s2, " \\ | \\ \\theta_1, \\theta_2) = \\begin{pmatrix} ", n1, " \\\\ ", s1," \\end{pmatrix} \\theta_1^{", s1, "}(1-\\theta_1)^{", n1, " - ", s1, "} \\cdot  \\begin{pmatrix} ", n2, " \\\\ ", s2," \\end{pmatrix} \\theta_2^{", s2, "}(1-\\theta_2)^{", n2, " - ", s2, "} \\, .$$"), fill=TRUE)
cat("<br>", fill=TRUE)
cat(paste0("Then $$L(\\theta_1, \\theta_2 \\ | \\ y_1=", s1, ",y_2=", s2, ")=\\begin{pmatrix} ", n1, " \\\\ ", s1," \\end{pmatrix} \\theta_1^{", s1, "}(1-\\theta_1)^{", n1, " - ", s1, "} \\cdot  \\begin{pmatrix} ", n2, " \\\\ ", s2," \\end{pmatrix} \\theta_2^{", s2, "}(1-\\theta_2)^{", n2, " - ", s2, "} \\, .$$"),fill=TRUE)
cat("<br>", fill=TRUE)
...
L1 = likelihood(s1/n1,s1,n1)*likelihood(s2/n2,s2,n2)
L2 = likelihood(0.5,s1,n1)*likelihood(0.5,s2,n2)

cat("<br>", fill=TRUE)
cat(paste0("\\begin{align} L \\biggl( \\theta_1= \\frac{", s1, "}{", n1, "}, \\theta_2= \\frac{", s2, "}{", n2, "} \\ \\biggl | \\ y_1=", s1, ", y_2=", s2, " \\biggl) &=", round(L1,4),", \\\\[1em] L(\\theta_1= 0.5, \\theta_2= 0.5 \\ | \\  y_1=", s1, ", y_2=", s2, ") &=", round(L2,4),".\\end{align}"), fill=TRUE)
cat("<br>", fill=TRUE)
...
cat("$$(\\theta_1, \\theta_2) \\sim p(\\theta_1,\\theta_2) = p(\\theta_1) \\cdot p(\\theta_2) = beta(a_1,b_1) \\cdot beta(a_2,b_2).$$",fill=TRUE)
...
if(a1!=0.5 | b1!=0.5 | a2!=0.5 | b2!=0.5){
  ...
  if(a1 == a2 & a1 == b1 & a1 == b2){
    cat("In this case, all parameters were chosen to be the same. This means it          was assumed that little or no information about the parameters to be           estimated was known in advance. The maximum of the prior distribution          is therefore at $(0.5, 0.5)$")
  }else{
    if(a1 >1 & b1 > 1){
    ...
    if(a2 >1 & b2 > 1){
      t2_m = round((a2-1)/(a2+b2-2))
    }else{
      t2_m = "not defined"
    }
    
    cat("In this case not all parameters were chosen to be the same. This means
        it was assumed that there were information about the parameters in
        advance. The prior is peaked in (",t1_m,",",t2_m,").")
  }
    
}else{
  cat("The default parameters are chosen: \n",fill=TRUE)
  cat("$$(\\theta_1, \\theta_2) \\sim  beta(0.5,0.5) \\cdot beta(0.5,0.5).$$",fill=TRUE)
  cat("These default parameters have their origin in the so called Jeffrey's prior which is a non-informative prior distribution. Non-informative prior distributions are used when little or no information is available about the parameters to be determined and therefore no statement can be made about the distribution of the parameters. In our assumed situation we have two parameters $\\theta_1$ and $\\theta_2$. In the following the Jeffrey's prior is described for one general $\\theta$, but we can use this results from Jeffrey's prior and insert them in both of the beta distributions multiplied in our prior. The priors $p(\\theta)=beta(0.5,0.5)$ result from Jeffrey's rule: $$p(\\theta) \\propto \\sqrt{I(\\theta)}$$ where $I(\\theta)$ is the Fisher information function $$I(\\theta)=-E\\left[ \\frac{\\partial^2}{\\partial \\theta^2} f(\\bullet \\ | \\ \\theta) \\right]$$ and where $f(\\bullet \\ | \\ \\theta)$ is the likelihood function of the data $\\bullet$.",fill=TRUE)
  cat("<br>", fill=TRUE)
  cat("In our case, the likelihood is a binomial distribution. Hence, the Fisher information function is $$I(\\theta)=\\frac{1}{\\theta (1-\\theta)} \\, .$$ This results in $$p(\\theta) \\propto \\theta^{-0.5}(1-\\theta)^{-0.5}$$ because of Jeffrey's rule. The resulting prior distribution is now proportional to $beta(0.5,0.5)$.",fill=TRUE)
...
cat("<br>", fill=TRUE)
cat("<br>", fill=TRUE)
cat("Notice when choosing the prior distribution, the amount of information already known about the probabilities $\\theta_1$ and $\\theta_2$ should be taken into account. If not so much or nothing at all is known, $a_i = b_i$ should be selected for the distribution of $\\theta_i$ $i=1,2$ so that the mean and the mode is equal at 0.5. In general the mean an the mode can be calculated as follows: $$ \\mu_i = \\frac{a_i}{a_i+b_i} \\text{ and } \\omega_i = \\frac{a_i -1}{a_i+b_i-2} \\text{ } a_i,b_i > 1$$",fill=TRUE)
cat("So if $a_i>b_i$ $\\mu_i$ and $\\omega_i$ are greater than 0.5 and if $a_i<b_i$ $\\mu_i$ and $\\omega_i$ are less than 0.5. So the prior distribution as product of two $beta(a_i,b_i)$ distributions is peaked in $(\\omega_1,\\omega_2)$, when $a_i,b_i > 1$ for all $i=1,2$, because all values of the beta density are greater or equal 0. The larger the sum $a_i+b_i$, the more certain one is in the assumption of the tendencies for the $\\theta_i$.")
...
pTheta1Theta2[is.infinite(pTheta1Theta2)] <- NA  # replaces infinity by NA
pTheta1Theta2 <- pmin(pTheta1Theta2, quantile(pTheta1Theta2, 0.99, na.rm = TRUE))
