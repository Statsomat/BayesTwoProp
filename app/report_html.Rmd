...
cat("=-0.1 suggests that, on average, the probability of ",name_outcome,"  in the group of ",name_exposure,"(\\(\\theta_1\\)) is slightly lower than in the group of ",name_nonexposure," (\\(\\theta_2\\)). However, this value alone is not sufficient to draw conclusions because it does not account for the uncertainty in the estimate. The HDI of", round(b$CI_low,3), "to ",round(b$CI_high,3) ,"indicates that there is a 95% probability that the true difference between infection probabilities ")
if (params$user_selection_function_param == 1) {
  cat("\\(\\theta_1 - \\theta_2\\)")
} else if (params$user_selection_function_param == 2) {
  cat("\\(\\frac{\\theta_1}{\\theta_2}\\)")
...
if (b$CI_high>0 && b$CI_low<0){
  cat(" Since this interval includes 0, it means that the evidence is inconclusive about whether exposure (\\(\\theta_1\\)) increases or decreases the probability of infection compared to non-exposure (\\(\\theta_2\\)).")
}else if (b$CI_high<0){
  cat(cat(" Since this interval lies entirely below 0, it suggests that exposure (\\(\\theta_1\\)) significantly decreases the probability of infection compared to non-exposure (\\(\\theta_2\\))."))
}else{
  cat(" Since this interval lies entirely above 0, it suggests that exposure (\\(\\theta_1\\)) significantly increases the probability of infection compared to non-exposure (\\(\\theta_2\\)).")
}
