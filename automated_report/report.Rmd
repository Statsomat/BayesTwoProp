...
perc_total <- paste(freq/totalsum*100,"%")
...
out_present <- paste(params$outcome,"=",params$presence_outcome)
out_not_present <- paste(params$outcome,"=",not_present_outcome)
...
```{r descplot fourfold, eval=eval_final, dev="cairo_pdf", results='asis', out.height= "40%", fig.align='center'}
cat("The next plot is a visualisation of the frequency table. \n", fill =TRUE)
fourfoldplot(table(df), color = c("#2fa42d","#396e9f"),conf.level = 0, margin = 1)

```

\pagebreak

```{r descplots, eval=eval_final, dev="cairo_pdf", results='asis', fig.cap='Barplots', fig.subcap=c("Barplot of Outcome", "Barplot of Exposure", "Barplot of Outcome with present Exposure", "Barplot of Outcome without present Exposure" ), fig.ncol=2, out.width="50%" ,fig.align='left', out.height="30%"}
cat("## Barplots", fill=TRUE)
out_vec <- df[params$outcome]
exp_vec <- df[params$exposure]
total <- nrow(df)

barplot(table(out_vec)/total, ylim=c(0,1), ylab= "Percentage of total Observations", col=c("#2fa42d","#396e9f")) 

barplot(table(exp_vec)/total,ylim=c(0,1), ylab ="Percentage of total Observations", col=c("#2fa42d","#396e9f"))


out_vec_exp1 <- out_vec[exp_vec==params$presence_exposure]
barplot(table(out_vec_exp1)/total,ylim=c(0,1), ylab ="Percentage of total Observations", col=c("#2fa42d","#396e9f"))

out_vec_exp0 <- out_vec[exp_vec!=params$presence_exposure]
barplot(table(out_vec_exp0)/total,ylim=c(0,1), ylab ="Percentage of total Observations", col=c("#2fa42d","#396e9f"))
...
cat("$$L_{n_1}(\\theta_1) = \\begin{pmatrix} n_1 \\\\ \\theta_1 \\end{pmatrix} \\theta_1^{y_1}(1-\\theta_1)^{n_1 - y_1}, \\qquad y_1, n_1 \\textrm{ given}$$", fill = TRUE)
cat("\\newline",fill=TRUE)
cat("\\newline",fill=TRUE) 
cat("$$L_{n_2}(\\theta_2) = \\begin{pmatrix} n_2 \\\\ \\theta_2 \\end{pmatrix} \\theta_2^{y_2}(1-\\theta_2)^{n_2 - y_2}, \\qquad y_2, n_2 \\textrm{ given}$$", fill = TRUE)
...
fig <- plot_ly(z = ~grid_likeli, x = ~theta1, y = ~theta2)
fig <- fig %>% add_surface()
...
cat("$\\theta_1 \\text{ \\~{ } } Beta(a_1,b_1)$ ")
cat("$\\theta_2 \\text{ \\~{ } } Beta(a_2, b_2)$")
