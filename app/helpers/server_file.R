...
server_file <- function(input, output, session) {
  ...
  # Upload message
  observeEvent(input$file, once=T, ignoreInit=T, {
...
### Upload Data ###
  datainput <- reactive({
    ...
    if (input$fencoding == "unknown"){
      validate(need(try(datainput1 <- fread(input$file$datapath, header = "auto", sep="auto", dec=".", encoding = "unknown", data.table = FALSE, na.strings = "")),
      ...
      validate(need(tryCatch(datainput1 <- fread(input$file$datapath, header = "auto", sep="auto", dec=".", encoding = "unknown", data.table = FALSE, na.strings = ""), warning=function(w) {}),
    ...
    if (input$fencoding == "UTF-8"){
      validate(
        need(guess_encoding(input$file$datapath)[[1]][1] %in% c("UTF-8","ASCII") & guess_encoding(input$file$datapath)[[2]][1] > 0.9,
             "Error. The file is probably not UTF-8 encoded. Please convert to UTF-8 or try the automatic encoding option.")
        )
      validate(need(try(datainput1 <- fread(input$file$datapath, header = "auto", sep="auto", dec=".", encoding = "UTF-8", data.table = FALSE, na.strings = "")), 
                    "Error. File cannot be read. Please check that the file is not empty, fully whitespace, or skip has been set after the last non-whitespace."))
      validate(need(tryCatch(datainput1 <- fread(input$file$datapath, header = "auto", sep="auto", dec=".", encoding = "unknown", data.table = FALSE, na.strings = ""), warning=function(w) {}),
                    "Error. The file cannot be read unambigously. Check the characters for the field separator, quote or decimal. Remove blank lines. "
                    ))
        ...
        datainput1 <- fread(input$file$datapath, header = "auto", sep="auto", dec="auto", encoding = "UTF-8", data.table = FALSE, na.strings = "")
          ...
          datainput1 <- fread(input$file$datapath, header = "auto", sep="auto", dec=input$decimal, encoding = "UTF-8", data.table = FALSE, na.strings = "")
            ...
            datainput1 <- fread(input$file$datapath, header = "auto", sep="auto", dec="auto", encoding = "unknown", data.table = FALSE, na.strings = "")
              ...
              datainput1 # Ohne diese Zeile stuerzt die App ab
              }
  ...
  output$selection_exposure <- renderUI({
    ...
    chooserInput("selection_exposure", "Available", "Selected",
                 ...
                 size = min(c(max(c(length(colnames(datainput())), selection_length_min)), selection_length_max)), multiple = FALSE)
    })
  ...
  # Render report
  observeEvent(input$generate, once=T, ignoreInit=T,{
