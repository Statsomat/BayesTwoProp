...
```{r descplot fourfold, eval=eval_final, results='asis', out.height= "40%", fig.align='center'}
...
```{r descplots, eval=eval_final, results='asis', fig.cap='Barplots', fig.subcap=c("Barplot of Outcome", "Barplot of Exposure", "Barplot of Outcome with present Exposure", "Barplot of Outcome without present Exposure" ), fig.ncol=2, out.width="50%", fig.align='left', out.height="30%"}
...
barplot(table(out_vec_exp1)/total,ylim=c(0,1), xlab="Outcome", ylab ="Proportion (under Exposure)", col=c("#2fa42d","#396e9f"))

#  Outcome without present Exposure
out_vec_exp0 <- out_vec[exp_vec!=params$presence_exposure]
barplot(table(out_vec_exp0)/total,ylim=c(0,1),  xlab="Outcome", ylab ="Proportion (without Exposure)", col=c("#2fa42d","#396e9f"))
...
cat("$$L(D_1 | \\theta_1) = \\begin{pmatrix} n_1 \\\\ \\theta_1 \\end{pmatrix} \\theta_1^{y_1}(1-\\theta_1)^{n_1 - y_1}, \\qquad D_1 = \\{y_1, n_1\\}$$", fill = TRUE)
cat("\\newline",fill=TRUE)
cat("\\newline",fill=TRUE) 
cat("$$L(D_2 | \\theta_2) = \\begin{pmatrix} n_2 \\\\ \\theta_2 \\end{pmatrix} \\theta_2^{y_2}(1-\\theta_2)^{n_2 - y_2}, \\qquad D_2 = \\{y_2, n_2\\}$$", fill = TRUE)
cat("\\newline",fill=TRUE)
cat("\\newline",fill=TRUE) 
cat("$$L(D | \\theta_1, \\theta_2) = \\begin{pmatrix} n_1 \\\\ \\theta_1 \\end{pmatrix} \\theta_1^{y_1}(1-\\theta_1)^{n_1 - y_1} \\begin{pmatrix} n_2 \\\\ \\theta_2 \\end{pmatrix} \\theta_2^{y_2}(1-\\theta_2)^{n_2 - y_2}, \\qquad D = \\{y_1, n_1, y_2, n_2\\}$$",fill=TRUE)
...
fig <- plot_ly(z = ~grid_likeli, x = ~Theta1, y = ~Theta2, contours = list(z = list(show = TRUE,
                                                                                    #usecolormap = TRUE,
                                                                                    #highlightcolor = "#ff0000",
                                                                                    project = list(z = TRUE))))

fig <- fig %>% add_surface(colorscale = list(c(0, 1), c("#2fa42d", "#396e9f")), hovertemplate = paste0('\u03b8\u2081: %{x}<br>', '\u03b8\u2082: %{y}<br>', 'L(D | \u03b8\u2081, \u03b8\u2082): %{z}<extra></extra>')) 

fig <- fig %>% layout(scene = list(camera = list(eye = list(x = -1.5, y = -1.5, z = 1.5)),
                                   xaxis=list(title = "\u03b8\u2081"), 
                                   yaxis=list(title = "\u03b8\u2082"), 
                                   zaxis=list(title = "L(D | \u03b8\u2081, \u03b8\u2082)"), 
                                   title = "theta"))
  

fig <- fig %>% layout(legend=list(title=list(text='<b> L(\u03b8\u2081, \u03b8\u2082) </b>')))

#layout(scene = list(xaxis=list(title = "&Theta;"), yaxis=list(title = "Test2"), zaxis=list(title = "Test3"), title = TeX("\\theta")) %>% config(mathjax = 'cdn'))
...
```{r posterior, eval=eval_final, results='asis'}
