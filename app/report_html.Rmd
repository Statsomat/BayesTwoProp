...
# Check for and handle invalid values in Likelihood
if (anyNA(Likelihood) || any(is.nan(Likelihood)) || any(is.infinite(Likelihood))) {
  cat("Warning: Invalid values found in Likelihood. Replacing with 0.\n")
  Likelihood[is.na(Likelihood) | is.nan(Likelihood) | is.infinite(Likelihood)] <- 0
}

# Check the range of Likelihood
if (all(Likelihood == 0)) {
  stop("Likelihood matrix is entirely zero. Check your likelihood calculations.")
}
