...
tryCatch({
  ...
  # n_exposure1_outcome0 = sum(df[params$outcome]!=params$presence_outcome & df[params$exposure]==params$presence_exposure) # maybe delete, no in use !!!
  # n_exposure0_outcome0 = sum(df[params$outcome]!=params$presence_outcome & df[params$exposure]!=params$presence_exposure) # maybe delete, no in use !!!
  # 
  ...
  # frequencies are called n_
  # for example n_exposure0_outcome1 is the frequency of datapoints without exposure but with present outcome
  s1 = sum(df[params$outcome]==params$presence_outcome & df[params$exposure]==params$presence_exposure) #s1
  s2 = sum(df[params$outcome]== params$presence_outcome & df[params$exposure]!=params$presence_exposure) #s2
  
  n1 = sum(df[params$exposure]==params$presence_exposure) #n1
  n2 = sum(df[params$exposure]!=params$presence_exposure) #n2
...
<!-- Dieser chunk hat folgende Variablen zur Verfügung: -->
<!-- exposurename -->
<!-- outcomename -->
...
<!-- Dieser chunk muss folgende Variablen erzeugen: (werden unten verwendet) -->
<!-- exposure -->
<!-- outcome -->
<!-- presence_outcome -->
<!-- presence_exposure -->
...
<!-- Dieser Chunk darf nur mit eval = evalfile laufen -->
...
<!-- Der chunk geht schief, weil man beim cell upload noch nicht params$outcome, params$exposure, params$presence_outcome, params$presence_exposure hat -->
...
<!-- params$exposure, params$outcome wird verwendet -->
...
\pagebreak
<!-- params$outcome, params$exposure, params$presence_outcome, params$presence_exposure -->
...
\pagebreak
<!-- hier wird params$data Ist das in Abhängigkeit von file upload? Wenn ja ist OK -->
...
cat("The likelihood function is a function of $(\\theta_1, \\theta_2 )$ that describes the plausibility of $(Y_1,Y_2)=(y_1,y_2)$ with different $(\\theta_1, \\theta_2 )$. The likelihood function should not be confused with a probability function. In our case, the binomial distribution (which is a discrete distribution) is used. Therefore, the likelihood function is a probability mass function. Note that the likelihood function can also be a density function if a continuous distribution is used.", fill = TRUE)
cat("<br>", fill=TRUE)
