...
if (is.null(rope_user)){
  a <- describe_posterior(df, ci = 0.95, centrality="all", dispersion=TRUE, test=NULL, ci_method = "ETI") 
  b <- describe_posterior(df, ci = 0.95, centrality="all", dispersion=TRUE, test=NULL, ci_method = "HDI")

} else {
  a <- describe_posterior(df, ci = 0.95, centrality="all", dispersion=TRUE, test="rope", rope_range=rope_user, ci_method = "ETI") 
  b <- describe_posterior(df, ci = 0.95, centrality="all", dispersion=TRUE, test="rope", rope_range=rope_user, ci_method = "HDI")
...
a2 <- data.frame(parameter =  rownames(a2)[-1], posterior = round(as.numeric(t(combined_results)[-1]),2))
